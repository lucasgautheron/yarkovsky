...
// matrice de rotation complète, plus rapide que le produit des trois
...
inline matrix tetrahedronmatrix(vec a, vec b, vec c)
{
    ...
    matrix transform(a, b, c); // jacobienne
    matrix transp = transform; transp.transpose();
    im.mul(cov, transp);
    double d = det3x3(transform); // déterminant de la jacobienne
    im.mul(transform, im);
    im *= d;
